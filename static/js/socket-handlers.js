...
const AudioFlowSocketHandlers = (function() {
    ...
    function handleNewFile(data) {
        ...
        if (typeof window.loadAudio === 'function') {
            window.loadAudio(data.filename, data.cover, displayFilename, data.title, data.artist, data.proxy_id, data.image_url);
        }
    ...
    function handleMemberCountUpdate(data) {
        const badge = document.querySelector('.member-count');
        if (badge) {
